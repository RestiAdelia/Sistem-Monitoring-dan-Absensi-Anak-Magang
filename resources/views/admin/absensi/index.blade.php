    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-1">Pusat pemantauan seluruh data log presensi harian anak magang dari semua instansi dan mentor.</p>
            </div>
        </div>
    </x-slot>
                ...
                <div class="px-6 py-5 border-b border-slate-100 bg-white">
                    <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <h3 class="text-base font-black text-slate-800 tracking-tight">Pelacakan Seluruh Log Absensi</h3>
                                <p class="text-xs text-slate-400 mt-0.5">Memantau riwayat kehadiran masuk dan pulang yang dikirim dari aplikasi Flutter anak magang.</p>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3 w-full xl:w-auto">
                            ...
                            <div class="relative flex-1 sm:w-60 sm:flex-none">
                                ...
                                <input type="text" x-model="search" placeholder="Cari nama, NIM, atau mentor..." class="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-700 placeholder-slate-400 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all duration-200 outline-none" />
                            </div>
                    ...
                    <table class="min-w-full border-collapse text-left">
                        <thead>
                            <tr class="bg-slate-100 border-b border-slate-300 text-xs font-bold uppercase tracking-wider text-slate-700 divide-x divide-slate-300">
                                ...
                                <th class="px-6 py-4">Koordinat Presensi</th>
                                <th class="px-6 py-4 text-center pr-8 w-28">Status</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-300">
                            @forelse($absensis as $index => $absen)
                                <tr class="hover:bg-slate-50 transition-colors duration-150 text-sm divide-x divide-slate-200"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex flex-col">
                                                <span class="font-bold text-slate-800 leading-none text-xs">{{ $absen->user->name }}</span>
                                                <span class="text-xs text-slate-400 font-mono mt-1 tracking-wide">NIM. {{ $absen->user->nomor_induk }}</span>
                                            </div>
                                        </div>
                                    </td>

                                    <td class="px-6 py-4 whitespace-nowrap text-xs text-slate-600 font-semibold">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-xs text-slate-600 font-mono font-semibold">
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="px-6 py-16 text-center bg-white text-slate-400 text-sm font-medium italic">
                                        Belum ada catatan aktivitas presensi dari seluruh anak magang di dalam sistem database.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
