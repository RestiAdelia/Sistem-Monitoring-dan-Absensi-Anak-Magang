    ...
    <div class="py-10 bg-slate-50/50 min-h-screen" x-data="{ search: '' }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow-sm rounded-2xl border border-slate-200 border-l-4 border-l-indigo-600 overflow-hidden">
                ...
                <div class="px-6 py-5 border-b border-slate-200 bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <tbody class="divide-y divide-slate-300">
                            @forelse($pendingAbsensis as $index => $absen)
                                <tr class="hover:bg-slate-50 transition-colors duration-150 text-xs divide-x divide-slate-300 border border-slate-300"
                                    ...
                                    <td class="px-4 py-3.5 border border-slate-300">
                                        @if($absen->lampiran)
                                            <a href="{{ asset('storage/' . $absen->lampiran) }}" target="_blank" class="font-bold text-indigo-600 hover:text-indigo-800 hover:underline flex items-center gap-1">
                                    ...
                                    <td class="px-4 py-3.5 border border-slate-300 bg-slate-50/30">
                                        <form action="{{ route('admin.absensi.action', $absen->id) }}" method="POST" class="flex items-center gap-1.5">
                                            ...
                                            <button type="submit" name="status_approval" value="approved" class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-2.5 py-1.5 rounded-lg transition-all shadow-sm">
                                                ACC
                                            </button>
                                            <button type="submit" name="status_approval" value="rejected" class="bg-rose-600 hover:bg-rose-700 text-white font-bold px-2.5 py-1.5 rounded-lg transition-all shadow-sm">
                                                Tolak
                                            </button>
                                        </form>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-10 text-center text-slate-400 italic bg-slate-50/50 font-medium border border-slate-300">
